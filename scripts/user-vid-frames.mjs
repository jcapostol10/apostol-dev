...
const videoPath = path.join(projectRoot, "VID_20260502_141729.mp4");
const tmpHtml = path.join(projectRoot, "_vidplay2.html");
writeFileSync(
  tmpHtml,
  `<!doctype html><html><body style="margin:0;background:#000">
<video id="v" src="VID_20260502_141729.mp4" muted preload="auto"></video>
<script>
...
mkdirSync("scripts/screens/userref", { recursive: true });
const n = 8;
for (let i = 0; i < n; i++) {
  const t = (meta.duration * (i + 0.5)) / n;
  await page.evaluate((t) => window._seek(t), t);
  await page.waitForTimeout(220);
  await page.locator("video").screenshot({ path: `scripts/screens/userref/frame-${i}.png` });
}
await browser.close();
console.log("ok");
